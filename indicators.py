...
def bollinger_bands(close: pd.Series, period: int = 20, std_factor: float = 2.0):
...
def adx(high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14):
    ...
    atr_n = tr.ewm(alpha=1 / period, adjust=False).mean().replace(0, np.nan)
    plus_di = 100 * pd.Series(plus_dm, index=high.index).ewm(alpha=1 / period, adjust=False).mean() / atr_n
    minus_di = 100 * pd.Series(minus_dm, index=high.index).ewm(alpha=1 / period, adjust=False).mean() / atr_n

    dx = (100 * (plus_di - minus_di).abs() / (plus_di + minus_di).replace(0, np.nan)).fillna(0.0)
    adx_v = dx.ewm(alpha=1 / period, adjust=False).mean().bfill()
...
def bullish_engulfing(open_: pd.Series, close: pd.Series) -> pd.Series:
    ...
    return (prev_bear & cur_bull & engulf).fillna(False)
...
def bearish_engulfing(open_: pd.Series, close: pd.Series) -> pd.Series:
    ...
    return (prev_bull & cur_bear & engulf).fillna(False)